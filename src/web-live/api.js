var Api = {
    ...
    getTodaysGame(cb) {
        ...
        var url = 'https://api.foxsports.com/sportsdata/v1/' + this.getSport(this.league) + '/' + this.league + '/teams/' + this.teamId + '/events.json?date=' + this.gameDate + '&apikey=' + this.apiKey;
        $.get(url).done(results => cb(results.page[0]));
    },

    getTeamInfo(teamId, cb) {
        var url = 'https://api.foxsports.com/sportsdata/v1/' + this.getSport(this.league) + '/' + this.league + '/teams/' + teamId + '.json?&apikey=' + this.apiKey;
        $.get(url).done(cb);
    },
